...
// Loaded once from the environment at bundle time — not user-configurable.
const OPENROUTER_KEY = (process.env.EXPO_PUBLIC_OPENROUTER_KEY ?? '').replace(/[\n\r\t​﻿]/g, '').trim();

// Default used when the user hasn't configured a model in Settings.
export const DEFAULT_AI_MODEL = 'qwen/qwen3-14b';

// Cheap reliable fallback — used only if the primary model fails.
const FALLBACK_MODEL = 'qwen/qwen3-8b';

function buildCascade(model: string): string[] {
  const primary = model || DEFAULT_AI_MODEL;
...
function getKey(): string {
  ...
  const prefix = OPENROUTER_KEY.slice(0, 10);
  const suffix = OPENROUTER_KEY.slice(-6);
  const isOrKey = OPENROUTER_KEY.startsWith('sk-or-v1-');
  console.log(`[OpenRouter] key: ${prefix}...${suffix} isOrKey=${isOrKey} len=${OPENROUTER_KEY.length}`);
  if (!isOrKey) console.warn(`[OpenRouter] WARNING: key does not start with sk-or-v1-`);
...
async function callOpenRouter(
  ...
  console.log(`[OpenRouter] → POST ${OPENROUTER_BASE}/chat/completions`);
  console.log(`[OpenRouter] → model: ${modelId}`);
  console.log(`[OpenRouter] → Authorization: Bearer ${prefix}...(hidden)...${suffix}`);
  console.log(`[OpenRouter] → messages: ${messages.length} (first role: ${messages[0]?.role})`);
  ...
  if (!response.ok) {
    ...
    console.error(`[OpenRouter] ← error body:`, JSON.stringify(body));
  ...
  console.log(`[OpenRouter] ← response len: ${text.length} chars, model: ${data.model ?? modelId}`);

  // Strip Qwen3 thinking blocks before returning
...
- Trust that the user has seen a lot. Be specific. Generic praise means nothing — say why it earns its place for THIS person.
...
- Include 3–5 titles.
- Open with ONE short sentence (max 15 words) that connects the user's request to their specific taste profile. Never generic openers like "Here are some recommendations" or "Based on your history".
- For each title: write the exact title name, then " — ", then one sentence (two max) explaining specifically why this viewer will love it. New line between each entry.
- Example reply format:
  "Your pull toward moral darkness and patient pacing makes these a natural fit:\n\nSicario — Your crime DNA puts you in Villeneuve's lane. The controlled dread is exactly what your most-watched titles share.\n\nThe Americans — Matches your high-prestige, slow-burn TV preference almost exactly."
      ...
      if (p && typeof p.reply === 'string') {
        ...
        if (inner.startsWith('{')) {
          try {
            ...
            if (p2?.reply) {
              const innerMovies = parseTitlesWithYears(Array.isArray(p2.movies) ? p2.movies : getStringArray(inner, 'movies'));
              const innerShows = parseTitlesWithYears(Array.isArray(p2.shows) ? p2.shows : getStringArray(inner, 'shows'));
              return {
...
export async function askAI(
  ...
  for (let i = 0; i < cascade.length; i++) {
    ...
    console.log(`[OpenRouter] Trying model ${i + 1}/${cascade.length}: ${modelId}`);
    try {
      const rawText = await callOpenRouter(key, modelId, chatMessages);
      const parsed = parseResponse(rawText);
      console.log(`[OpenRouter] Success: movies=${parsed.movies.length} shows=${parsed.shows.length}`);
      return { ...parsed, modelUsed: modelId };
    } catch (e) {
      lastError = e;
      const errMsg = String((e as any)?.message ?? '');
      console.warn(`[OpenRouter] Model ${modelId} failed: ${errMsg.slice(0, 200)}`);
...
Summary:`;
  ...
  for (const modelId of buildCascade(model)) {
    try {
      const text = await callOpenRouter(key, modelId, [{ role: 'user', content: prompt }]);
      if (text) return text;
    } catch {
      continue;
    }
